...
using Portfolio.Api.Settings.ModelValidator;
...
namespace Portfolio.Api.Settings.Services
{
  public class LocalizationService : ILocalizationService
  {
    ...
    public LocalizationService(SettingsDbContext dbContext, IMapper mapper, IMemoryCache memoryCache)
    {
    ...
    public async Task<Response<Localization>> Add(LocalizationAddDto localization)
    {
      try
      {
        ...
        bool exists = await _dbContext.Localizations.AnyAsync(f => f.Key == localization.Key);
        if (exists)
        {
        ...
        insertModel.Id = Guid.NewGuid().ToString();
        insertModel.CreatedDate = DateTime.Now;
        insertModel.CreatedUserId = 1;
        _dbContext.Localizations.Add(insertModel);
    ...
    public async Task<Response<NoContent>> Delete(string id)
    {
      try
      {
        Localization existing = await _dbContext.Localizations.FirstOrDefaultAsync(f => f.Id == id);
        if (existing != null)
        {
          _dbContext.Localizations.Remove(existing);
          await _dbContext.SaveChangesAsync();
        }
        _memoryCache.Remove(CACHE_KEY);
    ...
    public async Task<Response<List<Localization>>> GetAll()
    {
      try
      {
        List<Localization> localizations = await _dbContext.Localizations.ToListAsync();
    ...
    public async Task<Response<LocalizationGetByCultureDtoResponse>> GetByCulture(LocalizationGetByCultureDto getByCultureDto)
    {
      ...
      try
      {
        ...
        {
          localizations= await _dbContext.Localizations.Where(x => x.Key != null).ToListAsync();
        }
